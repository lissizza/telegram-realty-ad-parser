    ...
    def _generate_structured_prompt(self) -> str:
        ...
        if self.min_price is not None or self.max_price is not None:
            ...
            if self.min_price is not None:
                price_criteria.append(f"at least {self.min_price} {self.price_currency}")
            if self.max_price is not None:
                price_criteria.append(f"at most {self.max_price} {self.price_currency}")
            criteria_parts.append(f"Price ({self.price_currency}): {' and '.join(price_criteria)}")
